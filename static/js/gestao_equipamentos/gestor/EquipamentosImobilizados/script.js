...
addEquipmentForm.addEventListener('submit', (event) => {
    ...
    // Store data in dataset attributes for easy access later
    newRow.dataset.nome = nome;
    newRow.dataset.modelo = modelo;
    newRow.dataset.codigo = codigo;
    newRow.dataset.marca = marca;
    newRow.dataset.serie = serie;
    newRow.dataset.local = local;

    newRow.innerHTML = `
        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${nome}</td>
        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">${modelo}</td>
        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">${codigo}</td>
        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">${serie}</td>
        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 local-cell">${local}</td>
...
editEquipmentForm.addEventListener('submit', (event) => {
    event.preventDefault();
    const newLocal = document.getElementById('edit-equip-local').value;
    // Atualizar a célula da tabela
    rowToEdit.querySelector('.local-cell').textContent = newLocal;
    // Atualizar o dataset
    rowToEdit.dataset.local = newLocal;
    closeEditModal();
});
